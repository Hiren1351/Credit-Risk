...
loan['gender'].iloc[gender_null] = 'Male' #We fill missing value in gender by mode

loan.isnull().sum(axis = 0)

#Fill the missing Value in credit History Column

Counter(loan['ch'])#there is 475 1 and 89 0 so majority is 1 but still we want take any kind of risk because credit history is sensetive
loan['ch'].mode()

pd.crosstab(loan['ch'].isnull(),loan['status'])#we compare status(y-predicted or outcome variable) based on credit history

#We can see that there is 37 missing in yes and 13 missing in no
ch_null = loan[(loan['status'] == 'Y') & (loan['ch'].isnull())].index.tolist()
loan['ch'].iloc[ch_null] = 1 #out of 50 missing value 37 is from 1 so we simply replace with 1
...
loan['loanamt'].iloc[loanamt_null] = 146 #replace with mean

#married column has only 3 missing value so we drop

loan = loan.dropna()

#ch is a float but there is only two value 0 and 1 so we dont need to convert it

#Gender column is not impact much more for any bank to decide that give loan or not based on gender so drop it
...
X_train,X_test,Y_train,Y_test = train_test_split(X,Y,test_size = 0.3,random_state = 1234,stratify = Y)#we used stratified sampling 
...
#out of 184 record model predict 36 incorrect in which 34 is false positive that we want to descrese

#Create prediction Probability for checking threshold for sucess and failure

Y_prob = Lg.predict_proba(X_test)[:,1] #we check what %threshold for positive or 1

#lets set different threshold 
...
#out of 184 record model predict 57 incorrect in which 13 is false positive and 44 False Nagative that we want to descrese
...
#out of 184 record model predict 31 incorrect in which 26 is false positive and 05 False Nagative that we want to descrese
...
#out of 184 record model predict 36 incorrect in which 34 is false positive and 02 False Nagative that we want to descrese

#So,In Conclusion we can say that above 80% will aprove and below 60% reject
#Anything between 60-80 Hold

"""At 80% - total 95 approve out of 184
   At 60% - total 26 Reject out of 184
